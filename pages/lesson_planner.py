...
# # Standard template    # TODO use standard table to create generic table then called to create focus table and stages table
...
# Standard template
if "lesson_templates" not in st.session_state:
    st.session_state.lesson_templates = {
        "Standard":[
            {"type": "list", "title": "Aims / Objectives", "value": []},
            {"type": "list", "title": "Assumptions", "value": []},
            {"type": "list", "title": "Materials", "value": []},
...
with col1: 
    template_choice = st.selectbox(
        ...
        options=list(st.session_state.lesson_templates.keys()),
        index=0,
        key="lesson_template_choice"
    )

with col2: 
    if st.button("📋 Load Template", key="lesson_load_template", use_container_width=True):
        # Clear existing sections
        st.session_state.lesson_plan['sections'] = []
        
        # Load Standard template with empty values
        for section in st.session_state.lesson_templates["Standard"]:
            if section['type'] == 'stages_table':
                # Empty stages table
                new_section = {
                    "type": "stages_table",
...
# Main editing area
st.markdown("## Edit Lesson Plan")

# Quick add buttons - ALWAYS VISIBLE
st.markdown("### Add New Section")
col1, col2, col3 = st.columns(3)
with col1:
    if st.button("➕ Add List Section", key="lesson_add_list", use_container_width=True):
        st.session_state.lesson_plan["sections"].append({
            "type": "list", 
            "title": "New List", 
            "value": []
        })
        st.rerun()
with col2:
    if st.button("➕ Add Stages Table", key="lesson_add_stages", use_container_width=True):
        st.session_state.lesson_plan["sections"].append({
            "type": "stages_table", 
            "title": "Stages", 
            "stages": []
        ...
        st.rerun()
with col3:
    if st.button("➕ Add Target Language", key="lesson_add_focus", use_container_width=True):
        st.session_state.lesson_plan["sections"].append({
            "type": "focus_table", 
            "title": "Target Language", 
            "vocabulary": [],
...
if st.session_state.lesson_plan["sections"]:
    ...
    for idx, section in enumerate(st.session_state.lesson_plan["sections"]):
        with st.expander(f"📌 {section.get('title', 'Untitled')}", expanded=True):
            ...
            with col2:
                cols = st.columns([1,1,2])
                with cols[0]:
                    if idx > 0 and st.button("⬆️", key=f"lesson_up_{idx}"):
                        st.session_state.lesson_plan["sections"][idx], st.session_state.lesson_plan["sections"][idx-1] = \
                            st.session_state.lesson_plan["sections"][idx-1], st.session_state.lesson_plan["sections"][idx]
                        st.rerun()
                with cols[1]:
                    if idx < len(st.session_state.lesson_plan["sections"])-1 and st.button("⬇️", key=f"lesson_down_{idx}"):
                        st.session_state.lesson_plan["sections"][idx], st.session_state.lesson_plan["sections"][idx+1] = \
                            st.session_state.lesson_plan["sections"][idx+1], st.session_state.lesson_plan["sections"][idx]
                        st.rerun()
                with cols[2]:
                    if st.button("Delete", key=f"lesson_del_{idx}"):
                        st.session_state.lesson_plan["sections"].pop(idx)
                        st.rerun()
            ...
            with col1:
                # Edit section title
                new_title = st.text_input("Title", value=section.get('title', ''), 
                                        key=f"lesson_title_{idx}", label_visibility="collapsed")
                section['title'] = new_title
                ...
                if section['type'] == 'list':
                    ...
                    item_text = st.text_area(
                        "Items (one per line)", 
                        value="\n".join(items),
                        key=f"lesson_val_{idx}", height=100
                    )
                    section['value'] = [line.strip() for line in item_text.split('\n') if line.strip()]
                    ...
                    # Add stage button
                    if st.button("➕ Add Stage", key=f"lesson_add_stage_{idx}"):
                        stages.append({"name": "", "duration": "", "description": ""})
                        st.rerun()
                    ...
                    for s_idx, stage in enumerate(stages):
                        ...
                        with cols[0]:
                            stage['name'] = st.text_input("Stage", value=stage.get('name', ''),
                                                        key=f"lesson_stage_name_{idx}_{s_idx}")
                        with cols[1]:
                            stage['duration'] = st.text_input("Duration", value=stage.get('duration', ''),
                                                            key=f"lesson_stage_dur_{idx}_{s_idx}")
                        with cols[2]:
                            stage['description'] = st.text_area("Description", value=stage.get('description', ''),
                                                                key=f"lesson_stage_desc_{idx}_{s_idx}")
                        with cols[3]:
                            if st.button("🗑️", key=f"lesson_del_stage_{idx}_{s_idx}"):
                                stages.pop(s_idx)
                                st.rerun()
                    ...
                    vocab_text = st.text_area(
                        "Vocabulary (comma-separated)", 
                        value=", ".join(vocab_items),
                        key=f"lesson_vocab_{idx}", height=80
                    )
                    section['vocabulary'] = [v.strip() for v in vocab_text.split(',') if v.strip()]
                    ...
                    grammar_text = st.text_area(
                        "Grammar (comma-separated)", 
                        value=", ".join(grammar_items),
                        key=f"lesson_grammar_{idx}", height=80
                    )
                    section['grammar'] = [g.strip() for g in grammar_text.split(',') if g.strip()]
                    ...
                    section['functional_language'] = st.text_area(
                        "Functional Language",
                        value=func_text,
                        key=f"lesson_func_{idx}", height=100
                    )
else:
...
if st.button("Generate Document", type="primary", key="lesson_generate", use_container_width=True):
    with st.spinner("Creating your lesson plan..."):
        doc = format_document(st.session_state.lesson_plan)
